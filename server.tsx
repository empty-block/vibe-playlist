...
io.on('connection', (socket) => {
  ...
  socket.on('add-song', (data) => {
    ...
    if (user) {
      const songCount = db.query("SELECT COUNT(*) as count FROM songs WHERE playlist_id = ?").get(playlistId).count
      const newSong = {
        id: uuidv4(),
  ...
  socket.on('youtube-search', async (data) => {
    ...
    // Mock results for now - in production, use YouTube API
    const mockResults = [
      {
        videoId: 'dQw4w9WgXcQ',
...
server.listen(PORT, () => {
  console.log(`🎵 Vibes Playlist app (Bun + Solid.js) running on http://localhost:${PORT}`)
})